...
class EventDetailsInterfaceController: WKInterfaceController, WCSessionDelegate {
    ...
    @IBOutlet var progressView: WKInterfaceImage!
    ...
    override func awakeWithContext(context: AnyObject?) {
        ...
        if let eventDetailsValues = context as? NSArray {
            self.eventDetails = eventDetailsValues.mutableCopy() as? NSMutableArray
        }
    }
    ...
    override func table(table: WKInterfaceTable, didSelectRowAtIndex rowIndex: Int)
    {
        ...
        let h0 = {
            self.showProgressView(titleOfAlert!)
    ...
    {
        self.presentControllerWithName("progress", context: "Updating...")
    }
        ...
        self.performURLSessionForTaskForRequest(request, successBlock: { (response) -> Void in
            ...
            if let resultsValue = results.objectForKey("response"){
                if((resultsValue.valueForKey("statusCode")) as! NSNumber == 200){
                    ...
                    let copyDict = (self.eventDetails![indexpath] as! NSDictionary).mutableCopy()
                    copyDict.setValue(reminderId, forKey: "reminderId")
                    self.eventDetails?.replaceObjectAtIndex(indexpath, withObject: copyDict)
